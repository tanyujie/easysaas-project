package org.easymis.easysaas.common.mail;
/**
 * 简单邮件（不带附件的邮件）发送器
 */
...
public class SimpleMailSender {
	 ...
	 * @throws Exception
	 */
	public boolean sendAnnexMail(MailSenderInfo mailInfo, InputStream stream, String annexType) throws Exception {
		// 判断是否需要身份认证
		MyAuthenticator authenticator = null;
		Properties pro = mailInfo.getProperties();
		if (mailInfo.isValidate()) {
		...
		// 读取本地文件
		DataHandler dataHandler = new DataHandler(new ByteArrayDataSource(stream, annexType));
		attachment.setDataHandler(dataHandler);
		attachment.setFileName(MimeUtility.encodeText(mailInfo.getAttachFileNames()[0]));
		// 设置邮件消息发送的时间
		mailMessage.setSentDate(new Date());
		// 设置邮件消息的主要内容
		String mailContent = mailInfo.getContent();
	...
	public static void main(String[] args) throws Exception {
		...
		mailInfo.setFromAddress("itfather@1b23.com");
		mailInfo.setToAddress("tanyujie@sharepanzer.com");
